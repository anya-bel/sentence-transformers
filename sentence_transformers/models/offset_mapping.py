...
def create_offset_mapping(sentences, tokenized, tokenizer):
    ...
    :param sentences: texts that were tokenized
    :param tokenized: output of the Transformers tokenizer
    ...
    for num, sentence in enumerate(sentences):
        ...
        for elem in tokenized_tokens:
            ...
            while elem_pos < len(elem) and sentence[s_counter] == elem[elem_pos] and s_counter <= len(sentence):
                ...
                if s_counter > len(sentence) - 1:
                    break
            if s_counter < len(sentence) - 1 and sentence[s_counter] in [' ', '\u2005', '\u2008', '\u2009', '\u200a',
                                                                         '\u202f', '\xa0']:
                br.append(s_counter)
                s_counter += 1
            else:
                if s_counter < len(sentence) - 1 and sentence[s_counter + 1] not in [' ', '\u2005', '\u2008', '\u2009',
                                                                                     '\u200a', '\u202f', '\xa0']:
                    br.append(s_counter)
